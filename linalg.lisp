...
(defmethod m* ((m matrix) (s single-float))
  (make-instance 'matrix
		 :rows (matrix-rows m)
		 :cols (matrix-cols m)
		 :data (map '(simple-array single-float 1) #'(lambda (x) (* x s)) (matrix-data m))))
...
(defun is-threedimensional-vector-p (v)
...
(defun cross (a b)
  (assert (is-threedimensional-vector-p a))
  (assert (is-threedimensional-vector-p b))
  (make-instance 'matrix
		 :rows 3
		 :cols 1
